...
- (nullable NSArray<__kindof UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    [rowCollections enumerateKeysAndObjectsUsingBlock:^(id key, NSArray *itemAttributesCollection, BOOL *stop) {
        ...
        //算出每行第一个item的偏移量
        if (self.horizonalType == MLHorizonalLeft) {
            alignmentXOffset = sectionInsets.left;
        }else if (self.horizonalType == MLHorizonalCenter){
            alignmentXOffset = (collectionViewWidth - alignmentWidth) / 2.0;
        }else if (self.horizonalType == MLHorizonalRight){
...
- (CGSize)collectionViewContentSize
{
    ...
    if (self.collectionView.delegate && [self.collectionView.delegate respondsToSelector:@selector(collectionFlowLayout:didCalculatedContentSize:)]) {
        id<MLHorizontalCollectionViewFlowLayoutDelegate> delegate = (id<MLHorizontalCollectionViewFlowLayoutDelegate>)self.collectionView.delegate;
        [delegate collectionFlowLayout:self didCalculatedContentSize:size];
    }
    return size;
}
